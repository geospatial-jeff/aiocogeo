...
class Compressions:
    ...
    def decompress(self):
        ...
        try:
            ...
            raise NotImplementedError(f"{compression} is not currently supported") from e


    def _jpeg(self):
        jpeg_tables = self.ifd.JPEGTables
        jpeg_table_bytes = struct.pack(
            f"{self.reader._endian}{jpeg_tables.count}{jpeg_tables.tag_type.format}", *self.ifd.JPEGTables.value
        )
